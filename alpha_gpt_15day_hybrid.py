#!/usr/bin/env python3
"""
Alpha-GPT: 15-day Forward (Hybrid: Parallel + Checkpoint)
병렬 처리 + 세대마다 체크포인트 + 메모리 리프레시
"""
...
def genetic_programming_hybrid(seed_alphas, data, generations=30, population_size=100, num_workers=None):
    """하이브리드 GP: 병렬 + 체크포인트"""
    
    if num_workers is None:
        num_workers = 4  # Worker 4개 (1000 stocks)
    
    checkpoint_file = CHECKPOINT_DIR / "gp_checkpoint_hybrid.json"
    ...
    print(f"\n🧬 하이브리드 GP (병렬 + 체크포인트)")
    print(f"   세대: {start_gen}~{generations}, 개체수: {population_size}, 워커: {num_workers}")
...
def main():
    print("=" * 80)
    print("Alpha-GPT: 15-day Forward (Hybrid: Parallel + Checkpoint)")
    print("=" * 80)
    print()
    ...
    if seed_file.exists():
        ...
        print(f"    💾 Seed alphas 저장")
    
    # 하이브리드 GP
    best_alpha, best_ic = genetic_programming_hybrid(
        seed_alphas, 
        data, 
        generations=30, 
        population_size=100
    ...
    # 결과
    print("\n" + "=" * 80)
    print("🏆 BEST ALPHA (15-day forward, Hybrid)")
    print("=" * 80)
    ...
    if save.lower() == 'y':
        ...
        """, (
            best_alpha,
            float(best_ic),
            '15-day forward alpha (1000 stocks, hybrid: parallel + checkpoint, worker=4)'
        ))
