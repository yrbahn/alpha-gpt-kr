#!/usr/bin/env python3
"""
Alpha-GPT: 7-day (Weekly) Forward with GPT-4o
주간 리밸런싱용 알파 — Top 200 종목
"""
...
**Mode 1 - Light** (2 mutations): Fine-tune lookback windows only. If a parent uses ts_mean(x, 5), try 8 or 10.
  Consider: Weekly rebalancing (7d) favors shorter windows (3-60d).
...
foreign_net_ratio, inst_net_ratio, retail_net_ratio, foreign_ownership_pct"""
    ...
    prompt = f"""### Task
Generate {num_seeds} diverse alpha expressions for **7-day (weekly) forward returns** in KRX.
...
def _compute_ic_series(alpha_expr, data, date_start=None, date_end=None):
    ...
    forward_return = close.shift(-7) / close - 1  # 7영업일 (주간) 선행수익률
...
def main():
    print("=" * 80)
    print("Alpha-GPT: 7-day (Weekly) Forward with GPT-4o — Top 200")
    print("=" * 80)
    print()
    ...
    cv_folds = _make_cv_folds(close.index, n_folds=4, test_days=40, purge_days=10)
