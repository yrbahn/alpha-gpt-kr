#!/usr/bin/env python3
"""
Alpha-GPT: 15-day Forward (Simple, No Checkpoint)
간단하고 빠르게 - 체크포인트 없음
"""
...
def generate_seed_alphas():
    """GPT-4o로 초기 알파 생성"""
    print("\n🤖 GPT-4o로 초기 알파 생성 중...")
    
    client = openai.OpenAI(api_key=os.getenv('OPENAI_API_KEY'))
    
    prompt = """Generate 20 alpha factors for 15-day forward prediction.

Available: close, volume, returns
Operators: AlphaOperators.ts_delta(x, p), ts_mean(x, w), ts_std(x, w), ts_rank(x, w)
...
def genetic_programming(seed_alphas, data, generations=30, population_size=100):
    ...
    print(f"\n🧬 병렬 GP 시작")
    print(f"   세대: {generations}, 개체수: {population_size}, 워커: 4")
...
def main():
    print("=" * 80)
    print("Alpha-GPT: 15-day Forward (Simple)")
    print("=" * 80)
    print()
    ...
    # Seed 알파
    seed_alphas = generate_seed_alphas()
    ...
    if save.lower() == 'y':
        ...
        """, (
            best_alpha,
            float(best_ic),
            '15-day forward alpha (1000 stocks, simple, worker=4)'
        ))
